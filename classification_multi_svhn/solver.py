...
from dataset_comp.data_loader import load_data_multi
...
class Solver(object):
    def __init__(self, args, batch_size=64, source='svhn',
        ...
        if target != 'multi':
            tgt_list = [target]
        self.datadir = '/home/zhmiao/datasets/digits/'
    ...
    def train(self, epoch, record_file=None):
        ...
        joint_loader = zip(self.train_src_data, self.train_tgt_data)

        # for batch_idx, data in enumerate(self.datasets):
        for batch_idx, ((data_s, label_s), (data_t, _)) in enumerate(joint_loader):
